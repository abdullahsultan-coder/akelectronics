...
async function checkSession(){
  ...
  if (panelMode === "admin"){
    ...
    if (metaRole !== "products_agent" && metaRole !== "projects_agent" && metaRole !== "full_agent"){
      showScreen("denied");
      return;
    }
  ...
  if (CURRENT_ROLE === "admin" || CURRENT_ROLE === "products_agent" || CURRENT_ROLE === "full_agent") loadProducts();
  if (CURRENT_ROLE === "admin") loadStarterKits();
  if (CURRENT_ROLE === "admin" || CURRENT_ROLE === "projects_agent" || CURRENT_ROLE === "full_agent") loadProjects();
  ...
  if (CURRENT_ROLE !== "admin") loadAgentMessages(); // agents: personal notes from admin
...
function applyRolePermissions(){
  const allTabs = ["dashboard", "analytics", "aknexa", "products", "starterkits", "projects", "orders", "coupons", "messages", "agentmessages", "inquiries", "activitylog", "bundles", "blog", "media", "ads", "webleads"];
  let allowed = allTabs;
  if (CURRENT_ROLE === "products_agent") allowed = ["products", "orders", "inquiries", "coupons", "messages", "agentmessages"];
  if (CURRENT_ROLE === "projects_agent") allowed = ["projects", "orders", "inquiries", "coupons", "messages", "agentmessages"];
  if (CURRENT_ROLE === "full_agent") allowed = ["products", "projects", "orders", "inquiries", "coupons", "messages", "agentmessages"];
...
function wireTabs(){
  document.querySelectorAll(".tab-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (btn.dataset.tab === "agentmessages") loadAgentMessages();
...
async function loadOrders(){
  ...
  window.ORDERS_CACHE = {};
  data.forEach(o => { window.ORDERS_CACHE[o.id] = o; });
    ...
    return `
      <tr data-id="${o.id}" data-phone="${escapeAttr(o.phone || "")}" data-customer="${escapeAttr(o.customer_name || "")}">
        ...
        <td>
          ...
          <span class="row-msg"></span>
          <button type="button" class="icon-btn receipt-btn" style="width:100%; margin-top:4px;" data-id="${o.id}">🧾 Receipt PDF</button>
          <button type="button" class="icon-btn receipt-whatsapp-btn" style="width:100%; margin-top:4px;" data-id="${o.id}">📤 Send via WhatsApp</button>
  ...
  document.querySelectorAll("#ordersBody tr").forEach(tr => {
    ...
    if (receiptBtn) receiptBtn.addEventListener("click", () => generateOrderReceipt(receiptBtn.dataset.id, "download"));
    const receiptWaBtn = tr.querySelector(".receipt-whatsapp-btn");
    if (receiptWaBtn) receiptWaBtn.addEventListener("click", () => generateOrderReceipt(receiptWaBtn.dataset.id, "whatsapp"));
...
/* ---------------------------------------------------------
   ORDER RECEIPT — branded PDF (AK Electronics Pro) + WhatsApp share
   --------------------------------------------------------- */
function generateOrderReceipt(orderId, mode){
  const order = (window.ORDERS_CACHE || {})[orderId];
  if (!order){ alert("Order data not loaded — refresh the Orders tab and try again."); return; }
  ...
  doc.text("AK ELECTRONICS PRO", 14, 14);
  doc.setFontSize(9);
  doc.setFont(undefined, "normal");
  doc.text("Electronics  •  Components  •  Projects", 14, 21);
  doc.setFontSize(8);
  doc.text("Your Trusted Electronics Platform", pageWidth - 14, 14, { align: "right" });
  ...
  doc.text("Thank you for shopping with AK Electronics Pro! Your order has been received.", 14, 49);

  // ---- Info box ----
  doc.setDrawColor(225,225,235);
  doc.roundedRect(14, 56, pageWidth - 28, 26, 2, 2, "S");
  doc.setFontSize(9);
  doc.setTextColor(...grayText);
  doc.text("Customer Name", 20, 64);
  doc.text("Order ID", 90, 64);
  doc.text("Order Status", 150, 64);
  doc.setFontSize(10.5);
  doc.setTextColor(...dark);
  doc.setFont(undefined, "bold");
  doc.text(String(order.customer_name || "-"), 20, 70);
  doc.text("#AKP-" + shortId, 90, 70);
  doc.setTextColor(46,160,90);
  doc.text(String(order.status || "confirmed").toUpperCase(), 150, 70);

  doc.setFont(undefined, "normal");
  doc.setFontSize(9);
  doc.setTextColor(...grayText);
  doc.text("Phone / WhatsApp", 20, 78);
  doc.text("Order Date", 90, 78);
  doc.text("Delivery Method", 150, 78);
  doc.setFont(undefined, "bold");
  doc.setFontSize(10.5);
  doc.setTextColor(...dark);
  doc.text(String(order.phone || "-"), 20, 84);
  doc.text(new Date(order.created_at).toLocaleDateString(), 90, 84);
  doc.setFontSize(9);
  doc.text(String(order.payment_method || "Cash on Delivery"), 150, 84);
  ...
  doc.setFont(undefined, "bold");
  doc.text("#", 18, y + 6);
  doc.text("Product Name", 28, y + 6);
  doc.text("Qty", 130, y + 6);
  doc.text("Unit Price", 150, y + 6);
  ...
  items.forEach((it, idx) => {
    ...
    doc.text(String(it.product_name || "").slice(0, 45), 28, y);
    doc.text(String(it.quantity || 1), 130, y);
    doc.text(`${SITE_CONFIG.currency} ${Number(it.price || 0).toLocaleString("en-PK")}`, 150, y);
    doc.text(`${SITE_CONFIG.currency} ${lineTotal.toLocaleString("en-PK")}`, 178, y);
  ...
  y += 14;
  doc.setFillColor(...dark);
  doc.rect(pageWidth - 84, y - 7, 70, 12, "F");
  doc.setTextColor(255,255,255);
  doc.setFontSize(11);
  doc.setFont(undefined, "bold");
  doc.text("Total Amount", pageWidth - 80, y);
  doc.text(`${SITE_CONFIG.currency} ${Number(order.total || 0).toLocaleString("en-PK")}`, pageWidth - 18, y, { align: "right" });
  ...
  doc.setTextColor(...grayText);
  doc.setFont(undefined, "normal");
  doc.text(`WhatsApp: +${SITE_CONFIG.whatsappNumber}   |   Email: ${SITE_CONFIG.email}   |   ${SITE_CONFIG.websiteUrl || ""}`, 14, footerY - 3);
  doc.setFont(undefined, "italic");
  doc.text("Thank you for choosing AK Electronics Pro — 100% Original Products, Genuine & Reliable.", 14, footerY + 3);

  const fileName = `AK-Electronics-Pro-Receipt-${shortId}.pdf`;
  ...
  // WhatsApp mode: try native share (works on most mobile browsers) so the
  // PDF goes straight into a WhatsApp chat; otherwise fall back to
  // downloading the PDF + opening the customer's WhatsApp chat so it can be
  // attached manually (desktop browsers can't auto-attach files to WhatsApp Web).
  const pdfBlob = doc.output("blob");
  const pdfFile = new File([pdfBlob], fileName, { type: "application/pdf" });
  const waText = `Hi ${order.customer_name || ""}, here is your order receipt from AK Electronics Pro (#AKP-${shortId}). Thank you for shopping with us!`;

  if (navigator.canShare && navigator.canShare({ files: [pdfFile] })){
    navigator.share({ files: [pdfFile], title: "AK Electronics Pro — Order Receipt", text: waText })
      .catch(() => { /* user cancelled share sheet — no action needed */ });
  } else {
    doc.save(fileName);
    if (order.phone){
      const waLink = `https://wa.me/${order.phone.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(waText + " (Receipt PDF downloaded — please attach it here.)")}`;
      window.open(waLink, "_blank");
    } else {
      alert("Receipt downloaded. Customer phone number not on file — open WhatsApp manually to send it.");
    }
  }
}
...
/* ---------------------------------------------------------
   ANALYTICS (website traffic — visits, clicks, sources)
   --------------------------------------------------------- */
async function loadAnalytics(){
  const { data, error } = await supabase.from("analytics_events").select("*").order("created_at", { ascending: false }).limit(20000);
  if (error){ console.error(error); return; }

  const todayStart = new Date(); todayStart.setHours(0,0,0,0);
  const pageViews = data.filter(e => e.event_type === "page_view");
  const visitsToday = pageViews.filter(e => new Date(e.created_at) >= todayStart).length;
  const uniqueVisitors = new Set(pageViews.map(e => e.session_id)).size;
  const addToCart = data.filter(e => e.event_type === "add_to_cart").length;
  const whatsappClicks = data.filter(e => e.event_type === "whatsapp_click").length;
  const ordersPlaced = data.filter(e => e.event_type === "order_placed").length;

  document.getElementById("statVisitsToday").textContent = visitsToday;
  document.getElementById("statVisitsTotal").textContent = pageViews.length;
  document.getElementById("statUniqueVisitors").textContent = uniqueVisitors;
  document.getElementById("statAddToCart").textContent = addToCart;
  document.getElementById("statWhatsappClicks").textContent = whatsappClicks;
  document.getElementById("statOrdersPlaced").textContent = ordersPlaced;

  const convRate = pageViews.length ? ((ordersPlaced / pageViews.length) * 100).toFixed(1) + "%" : "0%";
  document.getElementById("ovTotalVisits").textContent = pageViews.length;
  document.getElementById("ovUniqueVisitors").textContent = uniqueVisitors;
  document.getElementById("ovConversionRate").textContent = convRate;
  document.getElementById("ovOrders").textContent = ordersPlaced;
  ...
  // Overview chart (Search-Console style): Visits (blue area) + Unique Visitors (purple line), last 30 days
  const ovDays = [];
  const ovVisits = [];
  const ovUnique = [];
  for (let i = 29; i >= 0; i--){
    ...
    ovDays.push(d.toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "2-digit" }).replace(/\//g,"/"));
    const dayEvents = pageViews.filter(e => { const t = new Date(e.created_at); return t >= d && t <= dEnd; });
    ovVisits.push(dayEvents.length);
    ovUnique.push(new Set(dayEvents.map(e => e.session_id)).size);
  }
  renderChart("overviewChart", {
    ...
    data: {
      labels: ovDays,
      datasets: [
        { label: "Visits", data: ovVisits, borderColor: "#4285f4", backgroundColor: "rgba(66,133,244,.18)", fill: true, tension: .25, pointRadius: 0, yAxisID: "y" },
        { label: "Unique Visitors", data: ovUnique, borderColor: "#7c5cff", backgroundColor: "rgba(124,92,255,.12)", fill: true, tension: .25, pointRadius: 0, yAxisID: "y1" }
      ]
    },
    options: {
      ...
      scales: {
        ...
        y: { position: "left", title: { display: true, text: "Visits" }, grid: { color: chartGridColor }, beginAtZero: true },
        y1: { position: "right", title: { display: true, text: "Unique Visitors" }, grid: { display: false }, beginAtZero: true }
      }
    }
  });

  // Visits — last 7 days (bar)
  const days = [];
  const counts = [];
  for (let i = 6; i >= 0; i--){
    const d = new Date(now); d.setDate(now.getDate() - i); d.setHours(0,0,0,0);
    ...
    counts.push(pageViews.filter(e => { const t = new Date(e.created_at); return t >= d && t <= dEnd; }).length);
  }
  renderChart("visitsChart", {
    type: "bar",
    data: { labels: days, datasets: [{ label: "Visits", data: counts, backgroundColor: "#4de3ff", borderRadius: 6 }] },
    options: { plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { grid: { color: chartGridColor }, beginAtZero: true } } }
  });

  // Traffic sources (bar)
  const sourceCounts = {};
  pageViews.forEach(e => { const s = e.referrer || "direct"; sourceCounts[s] = (sourceCounts[s] || 0) + 1; });
  const sourceLabels = Object.keys(sourceCounts);
  renderChart("trafficSourceChart", {
    type: "bar",
    data: { labels: sourceLabels, datasets: [{ label: "Visits", data: sourceLabels.map(s => sourceCounts[s]), backgroundColor: "#ff4d94", borderRadius: 6 }] },
    options: { plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { grid: { color: chartGridColor }, beginAtZero: true } } }
  });

  // Mobile vs Desktop (bar)
  const mobileCount = pageViews.filter(e => e.device_type === "mobile").length;
  const desktopCount = pageViews.filter(e => e.device_type === "desktop").length;
  renderChart("deviceChart", {
    type: "bar",
    data: { labels: ["Mobile", "Desktop"], datasets: [{ label: "Visits", data: [mobileCount, desktopCount], backgroundColor: ["#4de3ff", "#ff4d94"], borderRadius: 6 }] },
    options: { plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { grid: { color: chartGridColor }, beginAtZero: true } } }
  });

  // Funnel: Visits -> Add to Cart -> Orders (bar)
  renderChart("funnelChart", {
    type: "bar",
    data: { labels: ["Visits", "Add to Cart", "Orders Placed"], datasets: [{ label: "Count", data: [pageViews.length, addToCart, ordersPlaced], backgroundColor: ["#4de3ff", "#ffd24d", "#ff4d94"], borderRadius: 6 }] },
    options: { plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { grid: { color: chartGridColor }, beginAtZero: true } } }
  });
}

/* ---------------------------------------------------------
   AGENT MESSAGES (personal admin <-> agent notes)
   --------------------------------------------------------- */
async function loadAgentMessages(){
  const listEl = document.getElementById("agentMessagesList");
  const composeEl = document.getElementById("agentMessageCompose");
  if (!listEl) return; // tab not in DOM yet

  // Admin sees the compose box + every message; agents see only their own inbox (RLS handles filtering)
  if (composeEl) composeEl.style.display = (CURRENT_ROLE === "admin") ? "" : "none";

  const { data, error } = await supabase.from("agent_messages").select("*").order("created_at", { ascending: false });
  if (error){ listEl.innerHTML = `<p class="hint">Error: ${error.message}</p>`; return; }

  if (!data || !data.length){
    listEl.innerHTML = `<p class="hint">Koi message nahi hai.</p>`;
    return;
  }

  const roleLabel = { admin: "Admin", products_agent: "Products Team", projects_agent: "Projects Team", full_agent: "Full Agent" };
  listEl.innerHTML = data.map(m => {
    const fromLabel = roleLabel[m.from_role] || m.from_role;
    const toLabel = roleLabel[m.to_role] || m.to_role;
    const when = new Date(m.created_at).toLocaleString();
    const unreadBadge = (!m.is_read && CURRENT_ROLE !== "admin") ? `<span class="badge-unread">New</span>` : "";
    const isUrgent = (m.subject || "").toLowerCase().includes("urgent");
    return `
      <div class="agent-msg-item ${m.is_read ? "" : "unread"} ${isUrgent ? "urgent" : ""}" data-id="${m.id}">
        <div class="agent-msg-subject">${isUrgent ? "🔴" : "🔵"} ${(m.subject || "General Update").replace(/</g,"&lt;")}</div>
        <div class="agent-msg-meta"><strong>${fromLabel}</strong> → <strong>${toLabel}</strong> ${unreadBadge}<span class="agent-msg-time">${when}</span></div>
        <div class="agent-msg-text">${(m.message || "").replace(/</g,"&lt;")}</div>
      </div>
    `;
  }).join("");

  // Agents: mark their unread incoming messages as read once viewed
  if (CURRENT_ROLE !== "admin"){
    const unreadIds = data.filter(m => !m.is_read && m.to_role === CURRENT_ROLE).map(m => m.id);
    if (unreadIds.length){
      await supabase.from("agent_messages").update({ is_read: true }).in("id", unreadIds);
    }
  }
}

document.getElementById("sendAgentMessageBtn")?.addEventListener("click", async () => {
  const toRole = document.getElementById("agentMessageRecipient")?.value;
  const subject = document.getElementById("agentMessageSubject")?.value || "General Update";
  const text = document.getElementById("agentMessageText")?.value.trim();
  const msgEl = document.getElementById("agentMessageMsg");
  if (!text){ if (msgEl) msgEl.textContent = "Message likhna zaroori hai."; return; }
  ...
  if (error){ if (msgEl) msgEl.textContent = "Error: " + error.message; return; }
  if (msgEl) msgEl.textContent = "Bhej diya.";
  document.getElementById("agentMessageText").value = "";
  loadAgentMessages();
});
