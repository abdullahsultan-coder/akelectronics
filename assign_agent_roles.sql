...
-- OPTIONAL: a combined agent who can see BOTH Products Agent and
-- Projects Agent tabs in one login. Create this account in
-- Authentication > Users first (with its own email/password),
-- then update the email below and run this block.
update auth.users
set raw_user_meta_data = raw_user_meta_data || '{"role":"full_agent"}'::jsonb
where email = 'fullagent@gmail.com';

-- Confirm it worked — should show both emails with their role in the result:
select email, raw_user_meta_data
from auth.users
where email in ('productagent@gmail.com', 'projectagent@gmail.com', 'fullagent@gmail.com');
